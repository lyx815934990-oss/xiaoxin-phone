...
window.XiaoxinWeChatContacts = (function () {
        ...
        if (isFriend || isPendingRoleRequest || isSelf) {
            ...
            if (window.XiaoxinWeChatDataHandler && typeof window.XiaoxinWeChatDataHandler.getMoments === "function") {
                ...
                moments = allMoments.filter(function(m) {
                    ...
                    // 优先使用完整ID匹配，只有在完整ID不匹配时才使用去掉前缀后的匹配
                    var contactCharacterIdStr = String(contact.characterId || "").trim();
                    var contactCharacterIdBare = contactCharacterIdStr.replace(/^contact_/, "");
                    
                    // 1. 完整ID匹配（最严格）
                    if (authorStr === contactIdStr || authorStr === contactWechatIdStr) {
                        return true;
                    }
                    
                    // 2. 如果朋友圈author是纯数字，且联系人的characterId也是这个数字，才匹配
                    // 这样可以避免 author="101" 的朋友圈错误匹配到所有角色ID为101的联系人
                    if (/^\d+$/.test(authorBare) && contactCharacterIdStr && authorBare === contactCharacterIdBare) {
                        return true;
                    }
                    
                    // 3. contact_前缀匹配（兼容旧格式）
                    if (("contact_" + authorBare) === contactIdStr || ("contact_" + contactIdBare) === authorStr) {
                        return true;
                    }
                    
                    // 4. wechatId匹配（如果存在）
                    if (contactWechatIdStr && (authorStr === contactWechatIdStr || authorBare === contactWechatBare)) {
                        return true;
                    }
    ...
    function renderContactSettingsPage($root, mobilePhone, contact, source) {
        ...
        $editRemarkRow.on("click", function () {
            ...
            var $newRoot = $('<div class="xiaoxin-wechat-root"></div>');
            $container.append($newRoot);
            renderContactProfilePage($newRoot, mobilePhone, currentContact, source);
        });
            ...
            if (window.XiaoxinWeChatDataHandler && typeof window.XiaoxinWeChatDataHandler.addContact === "function") {
                ...
                setTimeout(function() {
                    ...
                    if ($container && $container.length) {
                        ...
                        // 检查当前是否在朋友资料页面，如果是则刷新朋友资料页面，否则刷新设置页面
                        var isProfilePage = $root.find(".xiaoxin-wechat-contact-profile-main").length > 0;
                        if (isProfilePage) {
                            renderContactProfilePage($newRoot, mobilePhone, updatedContactData, source);
                        } else {
    ...
    // ========== 渲染朋友资料页面 ==========
    function renderContactProfilePage($root, mobilePhone, contact, source) {
        console.info("[小馨手机][微信] 渲染朋友资料页面", contact);

        var $main = $(
            '<div class="xiaoxin-wechat-main xiaoxin-wechat-contact-profile-main is-fullscreen"></div>'
        );
        ...
        // 标题
        var $title = $('<div class="xiaoxin-wechat-header-title">朋友资料</div>');
        ...
        // 内容区域
        var $content = $('<div class="xiaoxin-wechat-contact-profile-content"></div>');
        ...
        // ========== 备注区域 ==========
        var $remarkSection = $('<div class="xiaoxin-wechat-contact-profile-section"></div>');
        var $remarkTitle = $('<div class="xiaoxin-wechat-contact-profile-section-title">备注</div>');
        $remarkSection.append($remarkTitle);

        // 备注名（支持实时编辑，打字机效果）
        var $remarkNameRow = $('<div class="xiaoxin-wechat-contact-profile-row xiaoxin-wechat-contact-profile-row-editable"></div>');
        $remarkNameRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">备注名</div>');
        var remarkName = currentContact.remark || currentContact.nickname || "";
        var $remarkNameValue = $('<div class="xiaoxin-wechat-contact-profile-row-value xiaoxin-wechat-contact-profile-row-value-editable">' + (remarkName || "") + '</div>');
        var $remarkNameInput = $('<input type="text" class="xiaoxin-wechat-contact-profile-row-input" value="' + (remarkName || "") + '" maxlength="20" style="display: none;">');
        $remarkNameRow.append($remarkNameValue);
        $remarkNameRow.append($remarkNameInput);
        $remarkNameRow.append(
            '<div class="xiaoxin-wechat-contact-profile-row-arrow">' +
                '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
                "</svg></div>"
        );
        ...
        $remarkNameRow.on("click", function () {
            if (isEditingRemark) return;
            isEditingRemark = true;
            
            // 隐藏显示值，显示输入框
            $remarkNameValue.hide();
            $remarkNameInput.show().focus();
            
            // 使用打字机效果输入当前值
            var currentValue = $remarkNameInput.val();
            $remarkNameInput.val("");
            
            if (window.gsap && currentValue) {
                // 使用 GSAP 实现打字机效果
                var chars = currentValue.split("");
                var index = 0;
                var typeInterval = setInterval(function() {
                    if (index < chars.length) {
                        $remarkNameInput.val($remarkNameInput.val() + chars[index]);
                        index++;
                    } else {
                ...
                // 如果没有 GSAP，直接设置值
                $remarkNameInput.val(currentValue);
            }
        });
        
        // 输入框失去焦点时保存
        $remarkNameInput.on("blur", function() {
            var newRemark = $remarkNameInput.val().trim();
            ...
            if (window.XiaoxinWeChatDataHandler && typeof window.XiaoxinWeChatDataHandler.addContact === "function") {
                ...
                // 更新显示值
                $remarkNameValue.text(newRemark || "");
                
                // 隐藏输入框，显示值
                $remarkNameInput.hide();
                $remarkNameValue.show();
        ...
        // 回车键保存
        $remarkNameInput.on("keydown", function(e) {
            if (e.key === "Enter" || e.keyCode === 13) {
                e.preventDefault();
                $remarkNameInput.blur();
            }
        });
        
        $remarkSection.append($remarkNameRow);

        // 电话
        var $phoneRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        $phoneRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">电话</div>');
        var phone = currentContact.phone || currentContact.phoneNumber || currentContact.电话号码 || "";
        var $phoneValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + (phone || "") + '</div>');
        $phoneRow.append($phoneValue);
        $phoneRow.append(
            '<div class="xiaoxin-wechat-contact-profile-row-arrow">' +
                '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
        ...
        $remarkSection.append($phoneRow);

        // 标签
        var $tagRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        $tagRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">标签</div>');
        var tags = Array.isArray(currentContact.tags) && currentContact.tags.length > 0 ? currentContact.tags : [];
        var $tagValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + (tags.length > 0 ? tags.join("、") : "") + '</div>');
        $tagRow.append($tagValue);
        $tagRow.append(
            '<div class="xiaoxin-wechat-contact-profile-row-arrow">' +
                '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
        ...
        $content.append($remarkSection);

        // ========== 权限区域 ==========
        var $permissionSection = $('<div class="xiaoxin-wechat-contact-profile-section"></div>');
        var $permissionTitle = $('<div class="xiaoxin-wechat-contact-profile-section-title">权限</div>');
        $permissionSection.append($permissionTitle);

        var $permissionRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        $permissionRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">权限</div>');
        var permissionText = "聊天、朋友圈";
        var $permissionValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + permissionText + '</div>');
        $permissionRow.append($permissionValue);
        ...
        $permissionSection.append($permissionRow);

        $content.append($permissionSection);

        // ========== 更多信息区域 ==========
        var $moreInfoSection = $('<div class="xiaoxin-wechat-contact-profile-section"></div>');
        var $moreInfoTitle = $('<div class="xiaoxin-wechat-contact-profile-section-title">更多信息</div>');
        $moreInfoSection.append($moreInfoTitle);

        // 我和他的共同群聊（根据性别动态显示人称代词）
        var $groupChatRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        var gender = currentContact.gender || currentContact.性别 || "";
        var genderText = gender === "女" || gender === "female" ? "她" : "他";
        $groupChatRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">我和' + genderText + '的共同群聊</div>');
        // TODO: 计算共同群聊数量（目前先显示固定值）
        var groupChatCount = 0; // 这里需要从数据中获取
        var $groupChatValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + groupChatCount + '个</div>');
        $groupChatRow.append($groupChatValue);
        $groupChatRow.append(
            '<div class="xiaoxin-wechat-contact-profile-row-arrow">' +
                '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                '<path d="M9 18L15 12L9 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
        ...
        $moreInfoSection.append($groupChatRow);

        // 签名
        var $signatureRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        $signatureRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">签名</div>');
        var signature = currentContact.signature || currentContact.个性签名 || "";
        var $signatureValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + (signature || "") + '</div>');
        $signatureRow.append($signatureValue);
        $moreInfoSection.append($signatureRow);

        // 添加时间（直接读取联系人中的好友添加成功时间）
        var $addTimeRow = $('<div class="xiaoxin-wechat-contact-profile-row"></div>');
        $addTimeRow.append('<div class="xiaoxin-wechat-contact-profile-row-label">添加时间</div>');
        var addTime = "";
        
        // ⚠️ 优先使用联系人中的 friendAcceptedRawTime（好友添加成功的原始时间字符串，更准确）
        // 然后使用 friendAcceptedAt（时间戳，作为备选）
        console.info("[小馨手机][微信] 解析添加时间，联系人数据:", {
            friendAcceptedAt: currentContact.friendAcceptedAt,
            friendAcceptedRawTime: currentContact.friendAcceptedRawTime,
            contactId: currentContact.id || currentContact.characterId
        });
        
        // ⚠️ 优先使用 friendAcceptedRawTime（原始时间字符串，更准确）
        if (currentContact.friendAcceptedRawTime) {
            try {
                var timeStr = String(currentContact.friendAcceptedRawTime).trim();
                console.info("[小馨手机][微信] 使用 friendAcceptedRawTime 解析添加时间，原始字符串:", timeStr);
                // 支持多种时间格式
                var cleanedTimeStr = timeStr
                    ...
                    .replace(/\s+/g, " ")
                    .trim();
                console.info("[小馨手机][微信] 清理后的时间字符串:", cleanedTimeStr);
                var parsed = Date.parse(cleanedTimeStr);
                if (!isNaN(parsed)) {
                    var date = new Date(parsed);
                    var year = date.getFullYear();
                    var month = date.getMonth() + 1;
                    addTime = year + "年" + month + "月";
                    console.info("[小馨手机][微信] 解析成功，添加时间:", addTime, "解析后的日期对象:", date);
                } else {
                    console.warn("[小馨手机][微信] 解析 friendAcceptedRawTime 失败，无法解析为日期:", cleanedTimeStr);
                }
            } catch (e) {
                console.warn("[小馨手机][微信] 解析friendAcceptedRawTime失败:", e);
            }
        }
        
        // 如果没有 friendAcceptedRawTime，尝试使用 friendAcceptedAt（时间戳，作为备选）
        if (!addTime && currentContact.friendAcceptedAt) {
            try {
                var date = new Date(currentContact.friendAcceptedAt);
                if (!isNaN(date.getTime())) {
                    var year = date.getFullYear();
                    var month = date.getMonth() + 1;
                    addTime = year + "年" + month + "月";
                    console.info("[小馨手机][微信] 使用 friendAcceptedAt 解析添加时间:", addTime, "原始时间戳:", currentContact.friendAcceptedAt);
                }
            } catch (e) {
                console.warn("[小馨手机][微信] 解析friendAcceptedAt失败:", e);
            }
        }
        
        // 如果仍然没有，尝试从好友申请记录中获取
        if (!addTime && window.XiaoxinWeChatDataHandler && typeof window.XiaoxinWeChatDataHandler.getFriendRequests === "function") {
            var requests = window.XiaoxinWeChatDataHandler.getFriendRequests() || [];
            var contactId = currentContact.id || currentContact.characterId || "";
            var relatedRequest = requests.find(function (req) {
                ...
                return (
                    (reqRoleIdStr === contactIdStr ||
                        reqRoleIdStr === "contact_" + contactIdStr ||
                        contactIdStr === "contact_" + reqRoleIdStr) &&
                    req.status === "accepted"
                );
            });
            if (relatedRequest && relatedRequest.rawTime) {
                console.info("[小馨手机][微信] 从好友申请记录中获取添加时间，rawTime:", relatedRequest.rawTime);
                // 解析时间字符串，格式化为"YYYY年M月"
                try {
                    var timeStr = String(relatedRequest.rawTime).trim();
                    // 支持多种时间格式
                    var cleanedTimeStr = timeStr
                        .replace(/-/g, "/")
                        .replace(/年/g, "/")
                    ...
                    if (!isNaN(parsed)) {
                        ...
                        var month = date.getMonth() + 1;
                        addTime = year + "年" + month + "月";
                        console.info("[小馨手机][微信] 从好友申请记录解析成功，添加时间:", addTime);
                    }
                } catch (e) {
                    console.warn("[小馨手机][微信] 解析添加时间失败:", e);
                }
            }
        }
        
        console.info("[小馨手机][微信] 最终添加时间:", addTime);
        
        var $addTimeValue = $('<div class="xiaoxin-wechat-contact-profile-row-value">' + (addTime || "") + '</div>');
        $addTimeRow.append($addTimeValue);
        $moreInfoSection.append($addTimeRow);

        $content.append($moreInfoSection);
